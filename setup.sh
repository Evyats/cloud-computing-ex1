...
echo
echo "The public IP of your new machine is: ${PUBLIC_IP}"
echo
echo "You can now send these requests to use the app:"
echo "curl -X POST \"http://<PUBLIC_IP>:5000/entry?plate=<PLATE_NUMBER>&parkingLot=<PARKING_LOT>\""
echo "curl -X POST \"http://<PUBLIC_IP>:5000/exit?ticketId=<TICKET_ID>\""
echo
echo "For example:"
echo "curl -X POST \"http://${PUBLIC_IP}:5000/entry?plate=123-123-123&parkingLot=382\""
echo "curl -X POST \"http://${PUBLIC_IP}:5000/exit?ticketId=1998\""
echo
